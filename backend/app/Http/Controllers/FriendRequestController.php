...
class FriendRequestController extends Controller
{
    ...
    {
        $user = auth()->user();
        $requests = $user->receivedFriendRequests()->where('is_denied', false)->with('senderUser')->get();

        return response()->json($requests);
    }

    public function acceptFriendRequest(FriendRequestActionsRequest $request)
    {
    ...
    public function denyFriendRequest(FriendRequestActionsRequest $request)
    {
        $friendRequest = FriendRequest::query()->where('id', $request->input('id'))->firstOrFail();
        $friendRequest->is_denied = true;
