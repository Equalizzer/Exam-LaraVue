...
positionableItems.click(function () {
    var target = '.' + $(this).data('target');
    var that = $(target);
    var di = parseInt(that.attr('data-rotate'));
...
function updateData() {
    ...
    reverseElements = $('.rotate-reverse');
    positionableItems = $(".circle-positionable");
    circleMenus = $(".circle-menu");
}
...
function initRoulette() {
    ...
    for (var i = 180; i > -180; i -= rotateStep) {
        ...
        dots.eq(dotsIndex).css({'left': coordinates.y, 'top': coordinates.x});
        dotsIndex++;
    }
...
function initPositions() {
    positionableItems.each(function (i, e) {
        ...
        $(e).position({of: target, my: 'center bottom', at: 'center top', collision: 'fit none'});
